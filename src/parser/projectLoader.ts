import { Project, ScriptTarget, type ProjectOptions, type SourceFile } from "ts-morph";
...
export class ProjectLoader {
    ...
    static fromTsConfig(tsConfigFilePath: string, options: LoaderOptions = {}): ProjectLoader {
    ...
    static fromDirectory(
        ...
        loader.project.addSourceFilesAtPaths(patterns.map((p) => `${directory}/${p}`));
        return loader;
    }

    static fromFiles(filePaths: readonly string[], options: LoaderOptions = {}): ProjectLoader {
        const loader = new ProjectLoader(options);
